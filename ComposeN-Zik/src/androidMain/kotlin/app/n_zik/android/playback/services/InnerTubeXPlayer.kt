...
import it.fast4x.innertube.models.Thumbnails
...
/**
 * The sole stream extraction entry point for N-Zik.
 *
 * Wraps InnerTubeX's [InnerTubeExtractor] to provide YouTube stream resolution
 * with automatic cipher management, replacing the legacy custom cipher pipeline.
 */
object InnerTubeXPlayer {
    ...
    /**
     * Resolves a playable audio stream for the given video ID.
     *
     * @param videoId YouTube video ID
     * @param playlistId Optional playlist ID for context
     * @param audioQuality Preferred audio quality
     * @param connectivityManager For network-aware quality selection
     * @param contentHints Content type hints for client selection
     * @param allowBoundedRange Whether to allow bounded range requests
     * @return Result containing [PlaybackData] or failure
     */
    suspend fun playerResponseForPlayback(
        videoId: String,
        playlistId: String? = null,
        audioQuality: InnerTubeXAudioQuality = InnerTubeXAudioQuality.AUTO,
    ...
    private suspend fun bundle(): ExtractionBundle {
        ...
        return bundleMutex.withLock {
            val latestTransport = Innertube.extractionTransport()
        ...
        return PlaybackData(
            audioConfig =
                if (loudnessDb != null || perceptualLoudnessDb != null) {
                    PlayerResponse.PlayerConfig.AudioConfig(loudnessDb?.toFloat(), perceptualLoudnessDb?.toFloat())
                } else {
                    null
                },
            videoDetails =
